...
export const AdminLayout = ({ children, currentTab = 'dashboard' }) => {
  ...
  return (
    <div className="min-h-screen bg-white text-neutral-900 flex flex-col lg:flex-row admin-view font-admin animate-fadeIn">
      {/* Admin Sidebar with Collapsible Rail on Desktop */}
      <aside className={`w-full ${isCollapsed ? 'lg:w-20' : 'lg:w-72'} bg-white border-r border-neutral-200 flex flex-col justify-between p-3.5 flex-shrink-0 transition-all duration-300`}>
        <div className="space-y-5">
          {/* Brand & Toggle Button */}
          <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} pb-3 border-b border-neutral-200`}>
            {!isCollapsed ? (
              <div className="flex items-center gap-3 min-w-0">
                <span className="w-9 h-9 rounded-xl bg-blue-600 text-white font-serif font-black flex items-center justify-center text-base shadow-sm flex-shrink-0">
                  H
                </span>
              ...
              <span className="w-10 h-10 rounded-xl bg-blue-600 text-white font-serif font-black flex items-center justify-center text-lg shadow-sm">
                H
              </span>
            )}
            ...
            <button
              ...
              className="hidden lg:flex p-1.5 hover:bg-neutral-100 text-neutral-500 hover:text-neutral-900 rounded-xl transition-colors"
              ...
              return (
                ...
                >
                  <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-2.5'}`}>
                    <Icon className={`w-4 h-4 ${isActive ? 'text-white' : item.highlight ? 'text-amber-500' : 'text-neutral-500'}`} />
                  ...
                  {!isCollapsed && item.highlight && !isActive && (
                    <span className="px-1.5 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200 text-[10px] font-mono font-bold">
                      ADSENSE
                    </span>
                  )}
                </button>
        ...
        <div className="pt-3 border-t border-neutral-200 space-y-2">
          {currentUser && (
            <div 
              ...
              className={`p-2 bg-neutral-50 hover:bg-neutral-100 cursor-pointer rounded-xl border border-neutral-200 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} text-xs transition-colors`}
              ...
              <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'space-x-2'} min-w-0`}>
                ...
                {!isCollapsed && (
                  <div className="min-w-0">
                    <p className="font-bold text-neutral-900 text-[11px] truncate">{currentUser.name}</p>
                    <p className="text-[10px] text-neutral-500 font-mono truncate">{currentUser.email}</p>
                  </div>
                )}
              </div>
              {!isCollapsed && currentUser.refCode && (
                <span className="px-1.5 py-0.5 bg-purple-50 text-purple-700 border border-purple-200 rounded text-[9px] font-mono font-bold">
          ...
          <button
            ...
            className={`w-full flex items-center ${isCollapsed ? 'justify-center py-2' : 'justify-between px-3 py-2'} text-xs font-semibold text-neutral-700 hover:bg-neutral-100 rounded-xl transition-colors border border-neutral-200 bg-white`}
          >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-white">
        ...
        <header className="h-14 bg-white border-b border-neutral-200 px-4 sm:px-6 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-2">
            <button
              onClick={toggleSidebar}
              className="p-1.5 hover:bg-neutral-100 text-neutral-500 hover:text-neutral-900 rounded-lg transition-colors mr-1"
            ...
            <span className="text-xs font-bold text-neutral-900 uppercase tracking-wider font-mono truncate max-w-[200px] sm:max-w-none">
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-xl text-[11px] font-mono text-emerald-700 shadow-xs">
              ...
              <span className="text-emerald-700 text-[10px]">Cloud MongoDB Ready</span>
            </div>

            {currentUser && (
              <button
                onClick={() => navigate('/admin/profile')}
                className="inline-flex items-center gap-2 px-3 py-1.5 bg-neutral-50 hover:bg-neutral-100 border border-neutral-200 text-neutral-800 rounded-xl text-xs font-mono transition-colors"
              ...
              >
                ...
                {currentUser.refCode && (
                  <span className="px-1 py-0.2 bg-purple-50 text-purple-700 rounded text-[9px] font-bold">
                    ?ref={currentUser.refCode}
                  </span>
                )}
              </button>
            ...
            <button
              onClick={logoutAdmin}
              className="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-xl text-xs font-bold font-mono transition-all flex items-center gap-1.5 border border-rose-200 active:scale-95 shadow-xs"
        ...
        <main className={`flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto bg-white ${isCollapsed ? 'max-w-full' : 'max-w-7xl'} w-full mx-auto transition-all`}>
          {children}
        </main>
      </div>
    </div>
