...
export const AdminStaffEditor = ({ staffId }) => {
  ...
  useEffect(() => {
    if (existingStaff) {
      setForm({
        ...
        username: existingStaff.username || (existingStaff.email ? existingStaff.email.split('@')[0] : 'user'),
        password: existingStaff.password || '123456',
        email: existingStaff.email || '',
        phone: existingStaff.phone || '',
        refCode: existingStaff.refCode || '',
        role: existingStaff.role || 'editor',
        roleName: existingStaff.roleName || 'Biên Tập Viên',
        joinDate: existingStaff.joinDate || new Date().toISOString().split('T')[0],
        status: existingStaff.status || 'active',
        avatar: existingStaff.avatar || AVATAR_PRESETS[0],
        baseSalary: existingStaff.salary?.baseSalary || 10000000,
        kpiBonus: existingStaff.salary?.kpiBonus || 0,
        permissions: existingStaff.permissions || {
          canManagePosts: true,
          canPublishPosts: existingStaff.role === 'admin',
          canManageCategories: existingStaff.role === 'admin',
          canViewRevenue: existingStaff.role === 'admin',
          canManageStaff: existingStaff.role === 'admin',
          canManagePayroll: existingStaff.role === 'admin',
          canManageComments: true,
          canManageSettings: existingStaff.role === 'admin'
        }
      });
    }
  }, [existingStaff]);
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      id: isEditing ? existingStaff.id : `staff-${Date.now()}`,
      name: form.name.trim(),
      ...
      salary: {
        ...
        deduction: existingStaff?.salary?.deduction || 0,
        payPeriod: '08/2026',
        paymentStatus: existingStaff?.salary?.paymentStatus || 'paid',
        paymentDate: new Date().toISOString().split('T')[0]
      }
    };

    saveStaff(payload);
    showToast(isEditing ? `Đã lưu cập nhật nhân viên "${form.name}"!` : `Đã thêm mới nhân sự "${form.name}" thành công!`, 'success');
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="font-serif text-2xl font-bold text-white">
              {isEditing ? `Chỉnh Sửa Hồ Sơ: ${existingStaff?.name || ''}` : 'Thêm Nhân Viên Mới Vào Tòa Soạn'}
            </h1>
            <p className="text-xs text-neutral-400">
              Thiết lập thông tin định danh, tài khoản đăng nhập CMS, mã Seeding và ma trận phân quyền chi tiết.
            </p>
          </div>
        </div>

        <div className="flex items-center space-x-2">
          {isEditing && existingStaff?.role !== 'admin' && (
            <button
              type="button"
          ...
          >
            ...
            <span>{isEditing ? 'Lưu Thay Đổi' : 'Tạo Nhân Viên Mới'}</span>
          </button>
        </div>
      </div>

      {/* Main Grid: 8 Cols Profile & Permissions + 4 Cols Credentials & Payroll */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* LEFT 7 COLS: Personal Info & Permissions Matrix */}
        ...
        {/* RIGHT 5 COLS: Credentials, Seeding Code & Payroll */}
        <div className="lg:col-span-5 space-y-6">
          ...
          {/* SECTION 4: Seeding Code & Referral Link */}
          <div className="p-6 bg-[#111726] rounded-3xl border border-[#1e293b] shadow-md space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <input
                  type="text"
                  placeholder="VD: MINH, AN, QB, LINH"
                  value={form.refCode}
          ...
          {/* SECTION 5: Salary & Payroll Config */}
          <div className="p-6 bg-[#111726] rounded-3xl border border-[#1e293b] shadow-md space-y-4">
            <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs uppercase font-mono pb-2 border-b border-[#1e293b]">
              ...
              <span>5. Bảng Lương & Đãi Ngộ</span>
            </div>
            ...
            <div className="space-y-3.5 text-xs">
              ...
              <div className="p-3 bg-[#182234] rounded-xl border border-[#2a3a54] flex items-center justify-between text-xs">
                <span className="text-neutral-400">Tổng Thu Nhập Dự Kiến:</span>
                <span className="font-mono font-bold text-emerald-400 text-sm">
                  {(Number(form.baseSalary) + Number(form.kpiBonus)).toLocaleString('vi-VN')} VND
                </span>
              </div>
            </div>
          </div>
